...
export default function HomePage() {
  return (
    <>
      ...
      <main>
        <style>{`
          .slot-card-link { text-decoration: none; display: block; }
          .slot-card { transition: transform 0.18s, border-color 0.18s; }
          .slot-card:hover { transform: translateY(-2px); border-color: rgba(200,244,0,0.2) !important; }
          .btn-g:hover { background: var(--green-dim) !important; transform: translateY(-1px); }
        ...
        <section className="hero-grid" style={{
          ...
          <div>
            <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--green)', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ width: '20px', height: '1px', background: 'var(--green)', display: 'block' }} />
              Globe Pitch · Bethnal Green
            </div>
            <h1 style={{ fontFamily: "'Archivo Black', sans-serif", fontSize: 'clamp(42px, 5vw, 68px)', lineHeight: 0.93, letterSpacing: '-2px', marginBottom: '1.25rem' }}>
              Fill the team.<br />
              <em style={{ fontStyle: 'normal', color: 'var(--green)', display: 'block' }}>Book the pitch.</em>
            </h1>
            <p style={{ fontSize: '16px', color: 'var(--muted)', lineHeight: 1.75, maxWidth: '400px', marginBottom: '2rem', fontWeight: 500 }}>
            ...
            <div style={{ display: 'flex', gap: '2rem', paddingTop: '2rem', borderTop: '1px solid var(--border)' }}>
              {[{ n: '10', l: 'players needed' }, { n: '£0', l: 'charged until full' }, { n: '4G', l: 'all-weather' }].map(item => (
                <div key={item.l}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[
              ...
              <Link key={card.time} href="/slots" className="slot-card-link">
                <div className="slot-card" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '14px', overflow: 'hidden', cursor: 'pointer' }}>
                  <div style={{ padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '3px 8px', borderRadius: '4px', marginBottom: '6px', display: 'inline-block', background: card.labelBg, color: card.labelColor }}>{card.label}</div>
                      <div style={{ fontFamily: "'Archivo Black', sans-serif", fontSize: '20px', letterSpacing: '-0.5px' }}>{card.time}</div>
                    </div>
                    <div>
                      <div style={{ fontFamily: "'Archivo Black', sans-serif", fontSize: '22px', color: 'var(--green)', letterSpacing: '-0.5px' }}>{card.pp}</div>
        ...
        <section style={{ maxWidth: '1100px', margin: '0 auto', padding: '4rem 1.5rem' }}>
          <div style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--green)', marginBottom: '0.75rem' }}>How it works</div>
          <div style={{ fontFamily: "'Archivo Black', sans-serif", fontSize: 'clamp(28px, 3.5vw, 42px)', letterSpacing: '-1px', marginBottom: '2.5rem' }}>Four steps to kickoff</div>
          <div className="steps-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
            {[
              ...
              <div key={step.n} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '10px', padding: '1.5rem' }}>
